...
const OFFER_ITEMS = [
  { t: 'Guia do Barbeiro — 9 módulos completos', tag: 'De R$ 49,90' },
  { t: 'Seção “Para o seu perfil” em cada módulo', tag: 'incluso' },
  { t: 'Referência rápida de ferramentas por categoria', tag: 'incluso' },
  { t: 'Acesso futuro ao app ManualChave — gratuito para quem comprar agora', tag: 'grátis' },
];
  ...
  <section id="oferta" style={{ background: 'var(--carvao-deep)', color: 'var(--branco)', padding: '140px 0', position: 'relative', overflow: 'hidden' }} className="grain">
    <div className="container-narrow" style={{ position: 'relative' }}>
      ...
      <div style={{
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {OFFER_ITEMS.map((it, i) => (
            <li key={i} style={{
              ...
              padding: '16px 0', borderBottom: i === OFFER_ITEMS.length - 1 ? 'none' : '1px solid #2a2a2a',
              alignItems: 'center',
            }}>
              <div style={{ color: 'var(--dourado)', fontFamily: 'var(--f-titulo)', fontWeight: 700, fontSize: 16 }}>✓</div>
              <div style={{ fontFamily: 'var(--f-corpo)', fontSize: 16, lineHeight: 1.55, color: 'rgba(255,255,255,.92)' }}>
                {it.app ? (
                  <span
                    style={{ position: 'relative', cursor: 'help', borderBottom: '1px dashed var(--dourado)' }}
                    onMouseEnter={e => { const tip = e.currentTarget.querySelector('.app-tip'); if (tip) tip.style.opacity = '1'; }}
                    onMouseLeave={e => { const tip = e.currentTarget.querySelector('.app-tip'); if (tip) tip.style.opacity = '0'; }}
                    onClick={e => { const tip = e.currentTarget.querySelector('.app-tip'); if (tip) tip.style.opacity = tip.style.opacity === '1' ? '0' : '1'; }}
                  >
                    {it.t}
                    <span className="app-tip" style={{
                      position: 'absolute', bottom: 'calc(100% + 12px)', left: 0,
                      width: 'min(360px, 80vw)', padding: '16px 18px',
                      background: 'var(--creme)', color: 'var(--carvao)',
                      fontFamily: 'var(--f-corpo)', fontSize: 13, lineHeight: 1.55,
                      ...
                      <div style={{ fontFamily: 'var(--f-titulo)', fontSize: 9, fontWeight: 700, color: 'var(--dourado-esc)', letterSpacing: '.16em', marginBottom: 8 }}>◆ BENEFÍCIO FUNDADOR</div>
                      Ao comprar, você garante <strong>acesso gratuito no lançamento do app</strong> — manual interativo + gerenciador básico para operar desde o primeiro dia.
                      <div style={{ marginTop: 8 }}>Desconto exclusivo no <strong>gerenciador PRO</strong>, com IA consultora e outras funcionalidades, quando lançar.</div>
                      <div style={{ marginTop: 8, fontStyle: 'italic', color: 'var(--dourado-esc)' }}>Benefício exclusivo para fundadores.</div>
                    </span>
                  </span>
                ) : it.t}
              </div>
              <div style={{
                fontFamily: 'var(--f-titulo)', fontSize: 10, fontWeight: 700,
                color: 'var(--dourado)', letterSpacing: '.14em', textTransform: 'uppercase',
                whiteSpace: 'nowrap',
              }}>[ {it.tag} ]</div>
            </li>
          ))}
        ...
        {/* Bônus card — "O Trabalho Vem Antes" */}
        <div style={{
          marginTop: 32,
          padding: '28px 32px',
          ...
          <div style={{ fontFamily: 'var(--f-titulo)', fontSize: 18, fontWeight: 700, color: 'var(--branco)', marginTop: 10, lineHeight: 1.4 }}>
            Material bônus — enviado 7 dias depois
          </div>
          <div style={{ fontFamily: 'var(--f-corpo)', fontSize: 15, color: 'rgba(255,255,255,.78)', marginTop: 10, lineHeight: 1.65 }}>
            Tem um material que não está à venda e não está descrito aqui. Você recebe sete dias depois da compra — quando a maioria das pessoas começa a questionar se vai conseguir. <strong style={{ color: 'var(--dourado)' }}>Isso não é coincidência.</strong>
          </div>
        </div>
        ...
        <div style={{ borderTop: '1.5px solid var(--dourado)', marginTop: 36, paddingTop: 32, textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 18 }}>
            ...
            <div className="display" style={{ fontSize: 'clamp(70px, 9vw, 120px)', color: 'var(--dourado)', fontStyle: 'italic', lineHeight: 1, marginTop: 6 }}>
              R$ 19,<span style={{ fontSize: '0.55em' }}>90</span>
            </div>
          ...
          <div style={{ marginTop: 32 }}>
            <a href="#cta-final" className="btn-primary" style={{ fontSize: 16, padding: '20px 36px' }}>
              QUERO O GUIA DO BARBEIRO — R$ 19,90 <span className="arrow">→</span>
            </a>
          </div>
          <div style={{ fontFamily: 'var(--f-titulo)', color: 'var(--cinza-medio)', fontSize: 12, marginTop: 18, letterSpacing: '.08em' }}>
            ✓ ACESSO IMEDIATO · ✓ 7 DIAS DE GARANTIA · ✓ PIX OU CARTÃO
          </div>
        </div>

        {/* Removed: locked-in future bonuses block (replaced by single bônus card above) */}
      </div>
    </div>
  </section>
);
...
const OBJECTIONS = [
  ...
  {
    q: '“R$ 19,90 ainda é dinheiro para mim agora.”',
    a: 'É justo avaliar. Mas pensa o outro lado: quantas horas você já gastou pesquisando sem sair do lugar? Esse tempo tem valor. E um erro em contrato, CNPJ ou precificação custa muito mais do que R$ 19,90 para corrigir. O guia existe para você gastar certo desde o início.',
  },
  {
  ...
  <section id="cta-final" style={{
    ...
    <div className="container-narrow" style={{ textAlign: 'center', position: 'relative' }}>
      ...
      <p style={{ fontFamily: 'var(--f-display)', fontStyle: 'italic', fontSize: 'clamp(22px, 2.6vw, 32px)', color: 'var(--dourado-clr)', marginTop: 32, fontWeight: 700 }}>
        Esse material existe. Custa R$ 19,90.
      </p>
      ...
      <div style={{
        ...
        <p style={{ fontFamily: 'var(--f-corpo)', fontSize: 16, lineHeight: 1.65, color: 'rgba(255,255,255,.85)', marginTop: 14 }}>
          Se servir — e vai servir — você vai olhar para trás e lembrar que tudo começou com uma decisão de R$ 19,90.
        </p>
      </div>
    </div>
  </section>
